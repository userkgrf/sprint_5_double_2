...
type CaloriesCalculator interface { // добавьте необходимые методы в интерфейс
	Calories() float64
	TrainingInfo() InfoMessage
}
...
func (w Walking) Calories() float64 {
	...
	return (CaloriesWeightMultiplier*w.Weight + (math.Pow(v, 2)/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
}
...
func ReadData(training CaloriesCalculator) string {
	...
	// добавьте полученные калории в структуру с информацией о тренировк
